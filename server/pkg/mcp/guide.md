...
- **`withHeaders()` is how the answer's headers are read**, and the only way:
  ...
  ```
- **A stream from the server is called like any other method**, and hands back
  the last message; every message is in the run's log. A method that streams from
  the client is not callable — `list_services` marks it, and calling it is
  refused whatever the request.
...
- `UNSUPPORTED` — Kaja refused the call before it went anywhere: the method
  streams from the client. `list_services` marks it `not callable`, and no
  request will change it.
