...
- **Two tools answer everything, and neither grows with the API.** `list_services` is an **index**: every app, service and method, each method's request/response type name, a `read`/`write` mark, and its HTTP request — filterable by `app`, `service` or free-text `search`. It never carries the generated sources; that is what overflowed an agent's context and forced it to grep a JSON blob. `describe_method "Shows.ListShows"` is the other half: the request type with every type it reaches **inlined transitively**, the response type, which fields the API declares required, and an **example call that runs as written** (`describe.go`, `example.go`). A miss doesn't just say no — an ambiguous name lists its candidates, an unknown one lists the nearest matches — because a dead end costs a whole round trip. Both answers are bounded: a type is cut off after `maxTypeLines`, a payload after `maxPayload`, and the cut says so.
- **A generated stub reads as source.** `createServiceInterfaceDefinition` prints its service object `multiLine`; the TypeScript printer puts a synthesized object literal on one line otherwise, so a thirty-method service arrived as one enormous line and any line-based reader — Monaco's hover, an agent grepping a stub — had to pull the whole module to find one signature.
- **The catalog is facts; the answer is written in Go.** `ui/src/mcpCatalog.ts` pushes what exists — services, methods, every message type reachable from one, fields with their marks and docs — and nothing about presentation. Whether a method reads or writes, how much of a type to show, and what an example looks like are decided in `desktop/mcp`, so the two can't disagree. Effect comes from the HTTP verb when there is one and from the method name when there isn't (`readingName`), and the listing marks the guess with a trailing `?` rather than passing it off as a fact.
- **`kaja.value` is named where it is needed.** A `google.protobuf.Value` field is left out of a generated request — there is no default that would send — so the only place it can be learned is at the point of use. `describe_method` annotates every such field with its builder and fills the example with `kaja.value(null)` / `kaja.struct({})` / `kaja.listValue([])`. Leaving this to the guide alone is what had an agent writing its own `str`/`bool` helpers for the `kind` oneof.
...
- **Desktop wiring** — `desktop/mcp_bridge.go` adapts the App's file methods to `mcp.Bridge`, generates the token, starts/stops the server on the `mcp:previewEnabled` toggle, and implements the run round-trip: `run_script` emits a `mcp:runScript` Wails event and waits on a channel; the UI runs the script and calls back the bound `MCPScriptResult`. The UI pushes the services catalog (`buildMcpCatalog`) via `MCPSetCatalog` as apps compile (and once when the server is enabled), covering every app uniformly, and shows the connection command via `MCPServerInfo`. Script mutations made through MCP tools emit a `mcp:scriptsChanged` Wails event; the UI live-reloads the content of an open script tab on `write_script` and keeps the sidebar list and open tabs in step with create/rename/delete, so agent edits show up without switching tabs.
