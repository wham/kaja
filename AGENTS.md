...
- Built by `scripts/server` into `server/build/protoc-gen-kaja`; shipped alongside the server binary in production and desktop builds
- At runtime, protoc-go is embedded as a Go library (imported as `github.com/wham/protoc-go/protoc`); only protoc-gen-kaja runs as a separate process. The protoc CLI is only used in development scripts.
