...
- `Configuration` is the file and nothing else. Facts about the running kaja — `can_update_configuration`, `git_ref`, `build_number`, `variable_store_available` — ride alongside it on `GetConfigurationResponse`, so saving never round-trips them. Older files carrying the retired `system` block have it stripped by `migrateConfiguration`, next to the legacy `projects` migration.
