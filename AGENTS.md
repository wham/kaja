...
- Don't update generated files directly; they will be overwritten.
  - `ui/src/server/` is generated by protoc-gen-ts (`npm run protoc` in ui/) or by protoc-gen-kaja in `scripts/server`. Both produce identical output today.
...
- **Two separate proto codegen use cases in Kaja:**
  - **Kaja's own API** (`server/proto/api.proto` → `ui/src/server/`): The Twirp connection between UI and server. Uses `@protobuf-ts/plugin` (protoc-gen-ts) in all environments. Run via `npm run protoc` in `ui/` or via `scripts/server`.
  - **User workspace protos** (compiled at runtime for the user's proto files): Uses protoc-gen-kaja in all environments. This is the Go plugin's primary purpose — compiling user proto files without requiring Node.js.
