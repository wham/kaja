...
- The UI uses shadcn/ui (Radix primitives + Tailwind CSS, stock neutral theme). Reusable primitives live in `ui/src/components/ui/`; compose them and style with Tailwind utility classes. Reach for a raw HTML element + Tailwind before adding a new primitive.
...
- Prefer the existing primitives in `ui/src/components/ui/` and Tailwind utilities; add a new shared primitive only when a pattern repeats across screens.
...
The UI is built on shadcn/ui — Radix primitives styled with Tailwind, stock
neutral theme. Reusable primitives live in `ui/src/components/ui/`; compose them
with Tailwind utilities rather than writing bespoke CSS. Add a new primitive only
when a pattern repeats.

### Available primitives (`ui/src/components/ui/`)
...
- **input** / **checkbox** / **switch** - form controls (Radix-backed where interactive)
- **select** - Radix Select (`SelectTrigger`, `SelectValue`, `SelectContent`, `SelectItem`, `SelectGroup`, `SelectLabel`)
...
- **confirmation-dialog** - `ConfirmationDialog` over Radix AlertDialog (`onClose(gesture)`)
- **dropdown-menu** - Radix DropdownMenu (`DropdownMenuTrigger`, `DropdownMenuContent`, `DropdownMenuItem`, …)
- **popover** - Radix Popover (`PopoverTrigger`, `PopoverContent`, `PopoverAnchor`)
- **tooltip** - Radix Tooltip plus `SimpleTooltip` (self-contained, `text`/`side`)
...
- Theme tokens: shadcn CSS variables plus a Primer-name alias bridge live in `ui/src/tailwind.css`. Day/night is a `dark` class toggled on `<html>` (so Radix portals are themed too); Monaco is themed separately via `vs`/`vs-dark`.
- Tailwind build: `ui/src/tailwind.css` is compiled by the Tailwind CLI (run through `bun`) inside an esbuild `OnStart` plugin in `server/internal/ui/builder.go`; the generated `ui/src/generated/tailwind.generated.css` is imported by `main.tsx` and folded into `main.css`.
