...
- `kaja.json` - Configuration file defining demo apps. The demo is **The Kaja Bakery**, three apps and three protocols hosted on kaja.tools: `bakebook` (OpenAPI, the book of what is baked), `oven` (gRPC, four racks and other bakers, live) and `kitchen` (MCP, the tools that scale a recipe, substitute an ingredient and pair a cookie with a drink). A cookie's `id` is the only identifier — it is what the oven and the kitchen both take. Beside them sit grpcb.in (gRPC) and quirks (Twirp), which are protocol testbeds rather than part of the demo.
- `quirks/proto/`, `grpcbin/proto/` - Proto files for the Twirp service and grpcb.in. Nothing else needs local protos: bakebook is OpenAPI (its spec is fetched from `spec_url` at runtime), oven uses gRPC server reflection, and kitchen's surface comes from the MCP server itself
- `scripts/` - The demo scripts, one per thing worth showing: `todays-cookies` (one call, one table), `whats-in-the-oven` (live racks with countdowns), `bake-a-batch` (all three apps, `askSelect`/`askInt`, and the two writes held behind `kaja.approve`) and `whats-missing` (a table whose rows are rewritten as the calls come back)
