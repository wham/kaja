...
- Don't update generated files directly; they will be overwritten.
  - `ui/src/server/` is generated by protoc-gen-ts (`npm run protoc` in ui/) for development, or by protoc-gen-kaja in `scripts/server` for production. Both produce identical output.
...
- Used at runtime for workspace proto compilation in production/desktop builds (no Node.js needed)
- **Important:** `ui/src/server/` has two codegen paths that produce identical output:
  - **Development:** `npm run protoc` in `ui/` uses `@protobuf-ts/plugin` (protoc-gen-ts, Node.js) — the original TS plugin is a production dependency in `ui/package.json`
  - **Production/CI:** `scripts/server` uses `protoc-gen-kaja` (Go) — no Node.js dependency needed for the final build
