...
- Theme tokens: the shadcn CSS variables live in `ui/src/tailwind.css`, stock neutral plus two additions, `--chrome` and `--brand-gradient`. Style with Tailwind utilities (`bg-muted`, `text-muted-foreground`, `border-border`). Day/night is a `dark` class toggled on `<html>` (so Base UI portals are themed too); Monaco is themed separately (see below).
...
- **Run is the brand button, and the only saturated fill in the window.** It carries `--brand-gradient`, which is the logo's own gradient (`docs/logo.svg`: rust → plum → violet, left to right, the same in both themes), so the brightest pixel in the app is a brand pixel. It used to be `bg-emerald-600`, which belonged to nothing and spent Kaja's one success colour on a button that is pressed before anything has succeeded — green now means only that a call succeeded. Hover brightens the whole gradient rather than moving one end of it. Nothing else takes this fill: a second brand-gradient element makes neither of them the brightest thing on the screen.
