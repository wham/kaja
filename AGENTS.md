...
- Surfaces: three planes, and a pane belongs to exactly one of them. **Content** — every tab body (editor, response viewer, compile log, app settings, variables) and the tab strip that names them — is `bg-background`, the deepest plane, so the thing you stare at longest is the quietest. **Chrome** — sidebar, top bar, status bar — is `bg-chrome`, one step above it. **Floating** — dialogs, popovers, menus, Monaco's suggest/hover widgets — is `bg-card`/`bg-popover`. `bg-muted` is a highlight for chips and pills, never a pane.
- Tabs (`ui/src/Tabs.tsx`): the strip is 38px of content plane with a single hairline under it — no cells, no vertical dividers, no accent colour, and the console header matches its height so the two rule the same line. A tab is `px-3` with no fill of its own: rest `text-muted-foreground`, hover `bg-accent/40 text-foreground`, active `text-foreground font-medium` plus a 2px `bg-foreground` rule inset 8px along its bottom, which is the whole of what marks it. Labels are `max-w-[180px]` then truncated, with the native title as tooltip; a preview tab is italic and loses the italic when it is kept. The close target is an 18×18 `rounded-sm` slot that is always in the layout — transparent at rest, so a label never shifts — and a dirty tab shows a 6px dot there that becomes the X when that slot is hovered. The row is one horizontally scrolling track with its scrollbar hidden and the active tab kept in view; the `</>` and `…` controls are pinned right on an opaque patch with a fade to their left, and no other control may live in the row. ⌃Tab / ⌃⇧Tab walk the tabs in the order they were last looked at and only settle that order when the modifier is released, ⌘1–9 jump by index, ⌘W closes the active tab, middle-click closes any tab, and dragging reorders within the strip (the tabs move as the pointer crosses them, so the underline sliding is the drop indicator).
