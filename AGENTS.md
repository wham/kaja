...
- **App form** — `AppForm.tsx` is the shell: the app picker, the Form/JSON toggle (the JSON editor opens on what the form currently holds, so switching views loses nothing), the **Advanced** disclosure that edits the headers every request carries, and the footer, whose left-hand sentence is the receipt of what the button will add. A type marked `customForm` renders its own body instead of the generic parameter list; its `parameters` list stays the contract with the config (which fields exist), and the custom form supplies the labels.
