...
func parseParameters(paramStr *string) params {
	...
	for _, name := range strings.Split(*paramStr, ",") {
		if strings.TrimSpace(name) == "force_optimize_code_size" {
			p.forceOptimizeCodeSize = true
		}
	}
	return p
}

// header is the credit line every generated file opens with, naming the parameters
// protobuf-ts would have been given. long_type_string is not a parameter here - it is
// what the plugin always generates - so it is always the first of them.
func (p params) header() string {
	names := []string{"long_type_string"}
	if p.forceOptimizeCodeSize {
		names = append(names, "force_optimize_code_size")
	}
	return "// @generated by protobuf-ts 2.11.1 with parameter " + strings.Join(names, ",")
}
...
// isOptimizeCodeSize returns true when the message types are generated without the
// speed-optimized create and binary codec, leaving the runtime to drive all three off
// the field descriptors. A file's own optimize_for says it, and the parameter
// force_optimize_code_size outranks whatever it says.
func (g *generator) isOptimizeCodeSize() bool {
	if g.params.forceOptimizeCodeSize {
		return true
	}
...
func (g *generator) writeImports(imports map[string]bool) {
	...
	if len(g.file.MessageType) > 0 {
		...
		// A wrapper's own JSON methods are generated either way, so this import is not the codec's.
