// Package kaja implements the protoc-gen-kaja plugin logic as a reusable library.
//
// This package can be used both as a subprocess plugin (via the protoc-gen-kaja
...
// NewPlugin returns a protoc-gen-kaja plugin that can be used in-process
// with [protoc.Result.RunLibraryPlugin].
func NewPlugin() protoc.Plugin {
	return protoc.PluginFunc(Generate)
}

// Generate processes a CodeGeneratorRequest and returns a CodeGeneratorResponse.
...
func (g *generator) collectLocalTypeNames() {
	...
	// Detect runtime BinaryReadOptions collision
	if g.localTypeNames["BinaryReadOptions"] {
		g.binaryReadOptionsRef = "BinaryReadOptions$"
	}
	...
	// Detect runtime JsonValue collision
	if g.localTypeNames["JsonValue"] {
		g.jsonValueRef = "JsonValue$"
	}
...
func (g *generator) binaryReadOptionsImport() string {
	if g.binaryReadOptionsRef == "BinaryReadOptions$" {
		return "BinaryReadOptions as BinaryReadOptions$"
	}
	return "BinaryReadOptions"
}

func (g *generator) binaryWriteOptionsImport() string {
	if g.binaryWriteOptionsRef == "BinaryWriteOptions$" {
...
func (g *generator) scalarTypeImport() string {
	if g.scalarTypeRef == "ScalarType$" {
		return "ScalarType as ScalarType$"
	}
	return "ScalarType"
}
...
func (g *generator) isJsonObjectImport() string {
	if g.isJsonObjectRef == "isJsonObject$" {
		return "isJsonObject as isJsonObject$"
	}
	return "isJsonObject"
}
...
func generateClientFile(file *descriptorpb.FileDescriptorProto, allFiles []*descriptorpb.FileDescriptorProto, params params) string {
	...
	// RPC imports
	if g.rpcTransportRef == "RpcTransport$" {
		g.pNoIndent("import type { RpcTransport as RpcTransport$ } from \"@protobuf-ts/runtime-rpc\";")
	} else {
	...
	if len(clientServices) > 0 {
		...
		if method0CallType != "" {
			...
			if hasUnaryInService && !seenCallTypes["UnaryCall"] {
				g.pNoIndent("import type { %s } from \"@protobuf-ts/runtime-rpc\";", g.callTypeImportClause("UnaryCall"))
				seenCallTypes["UnaryCall"] = true
			}
		}
...
func (g *generator) getMapKeyDefault(field *descriptorpb.FieldDescriptorProto) string {
	...
	case descriptorpb.FieldDescriptorProto_TYPE_INT32,
		...
		descriptorpb.FieldDescriptorProto_TYPE_SFIXED32,
		descriptorpb.FieldDescriptorProto_TYPE_SFIXED64:
		if field.GetType() == descriptorpb.FieldDescriptorProto_TYPE_INT64 ||
...
func (g *generator) generateWrapperMethods(typeName string) {
	...
	case "Int64Value":
		g.p("return this.refJsonWriter.scalar(%s.INT64, message.value, \"value\", false, true);", g.scalarTypeRef)
	case "UInt64Value":
		g.p("return this.refJsonWriter.scalar(%s.UINT64, message.value, \"value\", false, true);", g.scalarTypeRef)
	case "Int32Value":
...
func (g *generator) generateAnyMethods() {
	...
	g.p("if (typeName.startsWith(\"google.protobuf.\") || !%s(json))", g.isJsonObjectRef)
	g.indent = "            "
	g.p("json = { value: json };")
	g.indent = "        "
