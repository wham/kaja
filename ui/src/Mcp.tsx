 ...
 *
 * **The list is there whether the server is on or off.** What the switch decides is
 * whether anyone can reach the endpoint, never what it is or which agents can be
 * pointed at it — so the page reads the same either way, and the band's endpoint and
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden bg-background px-5 py-[18px]">
      <section aria-label="MCP server" className="flex shrink-0 flex-col gap-2 rounded-lg border border-border bg-card px-3.5 py-3">
        <div className="flex items-center gap-2">
          ...
          {/* The switch is the workspace's `mcp.enabled`, so where the workspace cannot be
              written there is nothing to offer: the state is stated and the verbs go, the
              rule every read-only screen is drawn under. */}
          ...
          {!readOnly && <Switch checked={control.enabled} onCheckedChange={(checked) => control.setEnabled(checked === true)} aria-label="Enable MCP server" />}
        </div>
