...
// The credential a gRPC app sends with every call. One per app, because one is
// what a service asks for; anything an API wants alongside it is a header.
...
// Where a surface came from, as the server reports it.
export const SOURCE_REFLECTION = "reflection";
export const SOURCE_PROTO_DIR = "proto_dir";

// Transport security. Empty leaves the decision to the URL, which is what kaja
// did before an app could say; the form settles it from what answered.
...
export interface AuthSchemeDefinition {
  key: string;
  label: string;
  // What the scheme is, in the words of the API dashboard the credential came
  // from.
  summary: string;
}

// The schemes, in the order they are offered. Bearer first: it is what most gRPC
// services ask for, and what an app that says nothing would have had to spell
// out as a header.
...
// apiKeyName is the metadata key an API key is sent under, which is lower-cased
...
// authNote says what kaja will do with the credential. Where that is a literal
// wire format, it is shown as one.
...
// isDialableTarget reports whether an address is one kaja could dial. It is
// deliberately loose - gRPC targets are written half a dozen ways, and the
// server is the one that finds out - so it only asks for a host.
export function isDialableTarget(value: string): boolean {
  if (holdsVariableReference(value)) return true;
  return targetHost(value) !== "";
}

// targetHost pulls the host out of a gRPC address, whichever of its several
// spellings was used: "dns:host:443", "dns:///host:443", "grpcs://host", or a
// bare "host:443".
...
// Labels that name where a service is deployed rather than what it is, so a name
// derived from a host doesn't come out as "api" or "grpc".
const HOST_NOISE = new Set(["api", "apis", "grpc", "grpcs", "rpc", "www", "svc", "service", "services", "dev", "staging", "stage", "prod", "test", "sandbox"]);

// Hosts that name the machine rather than the API. A name derived from one of
// them says nothing, so the services say it instead.
...
// deriveAppName turns what was typed into a name that reads as one word in an
// import path: "dns:seating.kaja.tools:443" becomes "seating", "grpcb.in:9000"
// stays "grpcb.in", and "localhost:50051" falls back to what the server said it
...
function nameFromHost(host: string): string {
  ...
  // "seating.kaja.tools" is named by its first label; "api.example.com" is named
  // by the one that follows it, since the first says only that it is an API.
...
function nameFromServices(services: string[]): string {
  ...
  // A service's package is what the API calls itself; the service's own name is
  // all there is when it has none.
...
// second server that names itself the same becomes "seating2".
...
// tlsFromServer is the transport the app should record after an inspection. A
// server that never answered settled nothing, so the app keeps leaving the
// decision to the URL.
...
// streamingMethods counts the methods that stream in either direction, which
// gRPC-Web can't carry.
